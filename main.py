...
from hash_functions import optimized_hash
...
def main():
    ...
    title_table = LinearProbingHashTable(table_size, optimized_hash)
    time_title = build_table(title_table, movies, "title")
    print_stats("Title Key - (Final Optimization)", title_table, time_title)

    # hash table 2 where the quote is the key (with linear probing and good hash)
    quote_table = LinearProbingHashTable(table_size, optimized_hash)
    time_quote = build_table(quote_table, movies, "quote")
    print_stats("Quote Key - Linear Probing (Final Optimization)", quote_table, time_quote)
